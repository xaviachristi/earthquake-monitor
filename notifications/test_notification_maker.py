...
class TestValidateKeys:
    ...
    def test_validate_keys_valid(self, sample_data, sample_non_usa_data, sample_non_country_data):
        ...
        assert validate_keys(sample_non_country_data)

    def test_validate_keys_no_topic(self, sample_data, sample_non_usa_data, sample_non_country_data):
        """Checks that the function returns False if the topic key
        doesn't exist."""
        sample_data.pop("topic_arn")
        sample_non_usa_data.pop("topic_arn")
        sample_non_country_data.pop("topic_arn")
        assert not validate_keys(sample_data)
        assert not validate_keys(sample_non_usa_data)
        assert not validate_keys(sample_non_country_data)

    def test_validate_keys_no_magnitude(self, sample_data, sample_non_usa_data, sample_non_country_data):
        """Checks that the function returns False if the magnitude key
        doesn't exist."""
        sample_data.pop("magnitude")
        sample_non_usa_data.pop("magnitude")
        sample_non_country_data.pop("magnitude")
        assert not validate_keys(sample_data)
        assert not validate_keys(sample_non_usa_data)
        assert not validate_keys(sample_non_country_data)

    def test_validate_keys_no_state(self, sample_data, sample_non_usa_data, sample_non_country_data):
        """Checks that the function returns False if the state key
        doesn't exist."""
        sample_data.pop("state_name")
        sample_non_usa_data.pop("state_name")
        sample_non_country_data.pop("state_name")
        assert not validate_keys(sample_data)
        assert not validate_keys(sample_non_usa_data)
        assert not validate_keys(sample_non_country_data)

    def test_validate_keys_no_region(self, sample_data, sample_non_usa_data, sample_non_country_data):
        """Checks that the function returns False if the region key
        doesn't exist."""
        sample_data.pop("region_name")
        sample_non_usa_data.pop("region_name")
        sample_non_country_data.pop("region_name")
        assert not validate_keys(sample_data)
        assert not validate_keys(sample_non_usa_data)
        assert not validate_keys(sample_non_country_data)

    def test_validate_keys_no_time(self, sample_data, sample_non_usa_data, sample_non_country_data):
        """Checks that the function returns False if the time key
        doesn't exist."""
        sample_data.pop("time")
        sample_non_usa_data.pop("time")
        sample_non_country_data.pop("time")
        assert not validate_keys(sample_data)
        assert not validate_keys(sample_non_usa_data)
        assert not validate_keys(sample_non_country_data)

    def test_validate_keys_no_tsunami(self, sample_data, sample_non_usa_data, sample_non_country_data):
        """Checks that the function returns False if the tsunami key
        doesn't exist."""
        sample_data.pop("tsunami")
        sample_non_usa_data.pop("tsunami")
        sample_non_country_data.pop("tsunami")
        assert not validate_keys(sample_data)
        assert not validate_keys(sample_non_usa_data)
        assert not validate_keys(sample_non_country_data)

    def test_validate_keys_no_latitude(self, sample_data, sample_non_usa_data, sample_non_country_data):
        """Checks that the function returns False if the latitude key
        doesn't exist."""
        sample_data.pop("latitude")
        sample_non_usa_data.pop("latitude")
        sample_non_country_data.pop("latitude")
        assert not validate_keys(sample_data)
        assert not validate_keys(sample_non_usa_data)
        assert not validate_keys(sample_non_country_data)

    def test_validate_keys_no_longitude(self, sample_data, sample_non_usa_data, sample_non_country_data):
        """Checks that the function returns False if the longitude key
        doesn't exist."""
        sample_data.pop("longitude")
        sample_non_usa_data.pop("longitude")
        sample_non_country_data.pop("longitude")
        assert not validate_keys(sample_data)
        assert not validate_keys(sample_non_usa_data)
        assert not validate_keys(sample_non_country_data)
...
class TestValidateTypes:
    ...
    def test_validate_types_valid(self, sample_data, sample_non_usa_data, sample_non_country_data):
        ...
        assert validate_types(sample_non_country_data)

    def test_validate_types_topic_invalid(self, sample_data, sample_non_usa_data, sample_non_country_data):
        ...
        sample_non_country_data["topic_arn"] = 3
        assert not validate_types(sample_data)
        assert not validate_types(sample_non_usa_data)
        assert not validate_types(sample_non_country_data)

    def test_validate_types_magnitude_invalid(self, sample_data, sample_non_usa_data, sample_non_country_data):
        """Checks that the function returns False if the magnitude isn't
        of the correct datatype."""
        sample_data["magnitude"] = 1
        sample_non_usa_data["magnitude"] = 2
        sample_non_country_data["magnitude"] = 3
        assert not validate_types(sample_data)
        assert not validate_types(sample_non_usa_data)
        assert not validate_types(sample_non_country_data)

    def test_validate_types_state_invalid(self, sample_data, sample_non_usa_data, sample_non_country_data):
        """Checks that the function returns False if the state isn't
        of the correct datatype."""
        sample_data["state_name"] = 1
        sample_non_usa_data["state_name"] = 2
        sample_non_country_data["state_name"] = 3
        assert not validate_types(sample_data)
        assert not validate_types(sample_non_usa_data)
        assert not validate_types(sample_non_country_data)

    def test_validate_types_region_invalid(self, sample_data, sample_non_usa_data, sample_non_country_data):
        """Checks that the function returns False if the region isn't
        of the correct datatype."""
        sample_data["region_name"] = 1
        sample_non_usa_data["region_name"] = 2
        sample_non_country_data["region_name"] = 3
        assert not validate_types(sample_data)
        assert not validate_types(sample_non_usa_data)
        assert not validate_types(sample_non_country_data)

    def test_validate_types_time_invalid(self, sample_data, sample_non_usa_data, sample_non_country_data):
        """Checks that the function returns False if the time isn't
        of the correct datatype."""
        sample_data["time"] = 1
        sample_non_usa_data["time"] = 2
        sample_non_country_data["time"] = 3
        assert not validate_types(sample_data)
        assert not validate_types(sample_non_usa_data)
        assert not validate_types(sample_non_country_data)

    def test_validate_types_tsunami_invalid(self, sample_data, sample_non_usa_data, sample_non_country_data):
        """Checks that the function returns False if the tsunami isn't
        of the correct datatype."""
        sample_data["tsunami"] = 1
        sample_non_usa_data["tsunami"] = 2
        sample_non_country_data["tsunami"] = 3
        assert not validate_types(sample_data)
        assert not validate_types(sample_non_usa_data)
        assert not validate_types(sample_non_country_data)

    def test_validate_types_latitude_invalid(self, sample_data, sample_non_usa_data, sample_non_country_data):
        """Checks that the function returns False if the latitude isn't
        of the correct datatype."""
        sample_data["latitude"] = 1
        sample_non_usa_data["latitude"] = 2
        sample_non_country_data["latitude"] = 3
        assert not validate_types(sample_data)
        assert not validate_types(sample_non_usa_data)
        assert not validate_types(sample_non_country_data)

    def test_validate_types_longitude_invalid(self, sample_data, sample_non_usa_data, sample_non_country_data):
        """Checks that the function returns False if the longitude isn't
        of the correct datatype."""
        sample_data["longitude"] = 1
        sample_non_usa_data["longitude"] = 2
        sample_non_country_data["longitude"] = 3
        assert not validate_types(sample_data)
        assert not validate_types(sample_non_usa_data)
        assert not validate_types(sample_non_country_data)
...
class TestMakeMessage:
    ...
    def test_make_message_returns_string(self, sample_data, sample_non_usa_data, sample_non_country_data):
        ...
        assert isinstance(make_message(sample_non_country_data), str)

    def test_make_message_tsunami_true(self, sample_data, sample_non_usa_data, sample_non_country_data):
        """Checks that tsunami information is included in the message
        if it is set to True."""
        sample_data["tsunami"] = True
        sample_non_usa_data["tsunami"] = True
        sample_non_country_data["tsunami"] = True
        assert "tsunami" in make_message(sample_data)
        assert "tsunami" in make_message(sample_non_usa_data)
        assert "tsunami" in make_message(sample_non_country_data)

    def test_make_message_tsunami_false(self, sample_data, sample_non_usa_data, sample_non_country_data):
        ...
        assert "tsunami" not in make_message(sample_non_country_data)
...
class TestGetLocationMessage:
    ...
    def test_get_location_message_usa(self, sample_data):
        ...
        assert result == f"in the area of {sample_data["state_name"]}, {sample_data["region_name"]}"
    ...
    def test_get_location_message_non_usa(self, sample_non_usa_data):
        ...
        assert result == f"in {sample_non_usa_data["region_name"]}"

    def test_get_location_message_non_country(self, sample_non_country_data):
        """Checks function returns expected data for earthquakes
        occurring outside sovereign territory."""
        result = get_location_message(sample_non_country_data)
        assert result == "outside sovereign territory"
